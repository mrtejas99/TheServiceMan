...
import { query,setDoc, collection, getDocs,getDoc,where,addDoc,Timestamp,onSnapshot,orderBy, collectionGroup,update,updateDoc, arrayUnion,doc,
...
function Chat() {
 ...
 const docSnap = await getDoc(doc(db, "last", id));
   if(docSnap.data())
   console.log(docSnap.data().to);
   console.log(user.uid);
        if (docSnap.data() && docSnap.data().to == user.uid) {
      // update last message doc, set unread to false
      await updateDoc(doc(db, "last", id), { unread: false });
    }
    ...
    const handleSubmit=async  e=>{ 
        ...
        //}
         ...
         /* in last collect when "to" == user.uid the unread=false*/
